          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
              {filtered.map((teacher) => (
                <tr key={teacher._id} className="hover:bg-gray-50/80 dark:hover:bg-zinc-800/80 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800/50 flex items-center justify-center text-indigo-700 dark:text-indigo-400 font-bold text-sm shrink-0">
                        {teacher.name.split(" ").map((n: string) => n[0].replace(/[^a-zA-Z]/g, "")).join("").substring(0, 2)}
                      </div>
                      <div>
                        <a href={`/admin/teachers/${teacher._id}`} className="font-semibold text-gray-900 dark:text-zinc-100 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{teacher.name}</a>
                        <div className="text-xs text-gray-500 dark:text-zinc-400">{teacher.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-gray-900 dark:text-zinc-200 font-medium">{teacher.subject}</div>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                      teacher.status === "Active" ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20"
                      : teacher.status === "On Leave" ? "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20"
